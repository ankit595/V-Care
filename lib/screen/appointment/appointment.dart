...
class _AppointState extends State<Appoint> {
  ...
  void bookAppointment() async {
    // Check if valid date and time are selected
    if (selectedIndex >= 0 &&
        selectedIndex < 8 && // Assuming 8 days are shown in the future
        selectedIndexHour >= 0 &&
        selectedIndexHour < timeSlots.length) {
      // Calculate selected date and time
      final DateTime selectedDate = DateTime.now().add(Duration(days: selectedIndex));
      ...
      try {
        ...
        String profilePictureUrl = patientData['patients_profile_picture'];
        // Create the appointment map with both doctor's and patient's details
        final Map<String, dynamic> appointment = {
          "userId": widget.doctorData['userId'],
          "doctorName": widget.doctorData['name'], // Access doctor's name from widget
          "doctorPicture": widget.doctorData['doctors_profile_picture'],
          "doctorSpecialty": widget.doctorData['specialty'],
          "patientName": patientData['name'], // Access patient's name from Firestore
        ...
        // Write appointment to local file
        ...
        // Appointment booked successfully, show dialog
        showDialog(
          context: context,
