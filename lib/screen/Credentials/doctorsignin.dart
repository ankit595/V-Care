...
class Doc_Signin extends StatefulWidget {
  final bool isPatient;
  const Doc_Signin({Key? key, this.isPatient = false}) : super(key: key);

  @override
  State<Doc_Signin> createState() => _Doc_SigninState();
}

class _Doc_SigninState extends State<Doc_Signin> with SingleTickerProviderStateMixin {
    ...
    try {
      ...
      if (userCredential.user != null) {
        ...
        if (user != null) {
          ...
          DocumentSnapshot userData = await FirebaseFirestore.instance.collection('doctors').doc(user.uid).get();
          if (userData.exists) {
            ...
            if (userMetadata.creationTime == userMetadata.lastSignInTime) {
              ...
              Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) => Doc_UserProfile()));
            } else {
